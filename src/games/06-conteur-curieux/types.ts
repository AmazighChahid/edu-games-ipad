 ...
 */
export type StoryTheme = 'nature' | 'adventure' | 'magic' | 'family' | 'friendship' | 'discovery' | 'fantasy';
...
/**
 * Badge de compétence gagné (type de données)
 */
...
/**
 * Scores de compétences pour le radar chart
 */
...
/**
 * Résultat d'une session de jeu (pour historique)
 */
export interface SessionResult {
  ...
  questionsTimeSeconds: number;
  hintsUsed: number;
  /** Scores par catégorie pour ce session */
  categoryScores: Partial<CompetencyScores>;
  /** Questions répondues par catégorie */
  questionsByCategory: Partial<Record<QuestionCategory, { correct: number; total: number }>>;
}

/**
 * Enregistrement audio de l'enfant
 */
...
export interface ParentSettings {
  ...
  preferredDifficulty: ConteurDifficulty;
  ...
  /** Temps max par session (minutes, 0 = illimité) */
  sessionTimeLimit: number;
}

/**
 * Données complètes du dashboard parent
 */
export interface ParentDashboardData {
  /** Nombre total d'histoires complétées */
  totalStoriesCompleted: number;
  /** Taux de réussite moyen */
  averageScore: number;
  /** Temps total de lecture (secondes) */
  totalReadingTime: number;
  /** Scores de compétences agrégés */
  competencyScores: CompetencyScores;
  /** Historique des sessions (dernières 20) */
...
/**
 * Configuration par défaut des paramètres parent
 */
...
/**
 * Compétences affichées (pour Victory screen)
 */
